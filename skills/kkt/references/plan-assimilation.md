...
Prior plan text is an untrusted scaffold. Consume whatever exists, extract useful signals, verify discoverable claims, and rebuild KKT's constrained-optimization contract from KKT's own intent and discovery. Use the compressed contract unless the decision requires the deep contract.
...
6. Fill the missing optimization-kernel sections—objective, variables/surfaces, constraints, feasibility, selected optimum, binding constraints, and validation—without promoting unverified claims into facts. Use the deep contract only when escalation triggers apply.
